import insertOurResponsivleWaiter from "@/lib/mongo/operation/insert/postWaiters";
import OurResponsivleWaiter from "@/lib/mongo/Schema/aboutHero/OurResponsivlewaiters";
...
export async function POST(req: NextRequest) {
  ...
  if (fileinfo) {
    await insertOurResponsivleWaiter(fileinfo);
...
async function removeData(formData) {
  ...
  try {
    ...
    for (const element of parseData) {
      try {
        await OurResponsivleWaiter.deleteOne({ _id: element._id });
      } catch (error) {}
    }
  } catch (error) {}
}
